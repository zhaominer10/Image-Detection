...
class AnchorsGenerator(nn.Module):
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0)):
        ...
        if not isinstance(sizes[0], (list, tuple)):
            sizes = tuple((s,) for s in sizes)  # ((128,), (256,), (512,))
        if not isinstance(aspect_ratios[0], (list, tuple)):
            aspect_ratios = (aspect_ratios,) * len(sizes)   # ((0.5, 1.0, 2.0), (0.5, 1.0, 2.0), (0.5, 1.0, 2.0))
        ...
        self._cache = {}    # 存储在原图上生成的所有anchors的坐标信息

    """
    generate_anchor函数目的是通过给定的anchors的比例，和anchors的面积(scale**2)，返回以原点(0,0)为中心的所有anchor坐标
    """
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        # 用以上scales与aspect_ratio为例，ws添加None为向量运算，将tensor(3,) converto to tensor(3,1)
        # scales添加None为向量运算，将tensor(3,) converto to tensor(1,3),点积得到tensor(3,3),最后通过view(-1)得到tensor(9,)
    ...
    def forward(self, image_list, feature_maps):
        ...
        # type example [torch.Size([500, 486])]
...
class RPNHead(nn.Module):
    ...
    """
    x 为backbone返回的预测特征层
    faster rcnn原论文只是用了一个预测特征层
    在train_mobilenet2v也只采用了一个预测特征层
    在ResNet50 FPN中采用了多个预测特征层，在不同的预测特征层上去分别生成目标分类分数和目标框回归参数
    """
    def forward(self, x):
