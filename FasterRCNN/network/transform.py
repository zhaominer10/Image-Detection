...
# TODO torch.jit: 关于TorchScript 与 TorchModule
@torch.jit.unused
...
def _resize_image(image, self_min_size, self_max_size):
    ...
    # 先将小边缩放至最小值，如果这个缩放比例不能将大边缩放至self_max_size内，则说明缩放的比例还不够，还需继续缩放
    # 因此将大边缩放至self_max_size，这个缩放比例应该更强烈
    # 这个最后的结果是，优先将图片缩放至最小边长要求，如果此时最大边长要求不满足，则继续缩放，至最大边长满足要求，这时最小边长要求可能会不满足
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    """
    image_mean, image_std should be tuple datatype, e.g.,(3,)
    """

    def __init__(self, min_size, max_size, image_mean, image_std):
        super(GeneralizedRCNNTransform, self).__init__()
        if not isinstance(min_size, (list, tuple)):
            min_size = (min_size,)  # 如果条件不满足，则将min_size转换为tuple格式
    ...
    def resize(self, image, target):
        ...
        if self.training:
            ...
            # 随机返回self.min_size中的某一元素
            # 在本FastRCNN项目中，传入的min_size都是int，所以self.min_size都只有一个元素
            # 这么写的原因是在其他地方有扩展用法，此不赘述
        else:
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        # 这里images[0]的选择不重要，随便初始化一个tensor，然后使用new_full transfrom为目标shape的tensor，并且指定tensor value
    ...
    def postprocess(self,
        ...
        Args:
            result: list(dict), 网络的预测结果, len(result) == batch_size, 包含了bbox信息及其类别信息
...
def resize_boxes(boxes, original_size, new_size):
    ...
    ]  # e.g., ratios = [tensor(0.5000), tensor(0.5000)]
